...
const Pagination: React.FC<PaginationProps> = ({ pageNumbers, setCurrentPage }) => {
            ...
            <nav >
                <ul className="flex items-center -space-x-px h-10 text-base">
                    ...
                    {pageNumbers.map((page: number, index: number) => (
                        <li key={index}>
                            <button onClick={() => handleClick(page)} className="flex items-center justify-center px-4 h-10 leading-tight border bg-gray-800 border-gray-700 text-gray-400 hover:bg-gray-700 hover:text-white">{page}</button>
                        </li>
                    ))}
